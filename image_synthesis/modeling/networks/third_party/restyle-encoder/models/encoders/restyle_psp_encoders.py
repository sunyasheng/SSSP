...
from .map2style import GradualStyleBlock
...
class ResNetBackboneEncoder(Module):
    ...
    def __init__(self, n_styles=18, input_nc=3):
        ...
        ## for contrastive computing
        # self.blocks = [Sequential([self.conv1, self.bn1, self.relu])] + blocks
...
class ResNetBackboneEncoderWCamHead(ResNetBackboneEncoder):
    """
    The simpler backbone architecture used by ReStyle where all style vectors are extracted from the final 16x16 feature
    map of the encoder. This classes uses the simplified architecture applied over an ResNet34 backbone.
    """
    def __init__(self, n_styles=18, input_nc=3):
        super(ResNetBackboneEncoderWCamHead, self).__init__(n_styles=n_styles, input_nc=input_nc)
        self.cam_head = nn.Sequential(nn.Linear(512, 24), 
                                      nn.ReLU(),
                                      nn.Linear(24, 6))

    def forward(self, x):
        # directly copy from his ancestors
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.body(x)
        ...
        # a head is implemented here for cam pose prediction
        cam_out = self.cam_head(out)
