...
def main(rank, opt, name='examples'):
    ...
    for i in range(len(im_path)):
        ...
        mat_path = os.path.join(visualizer.img_dir, name.split(os.path.sep)[-1], 'epoch_%s_%06d'%(opt.epoch, 0),img_name+'.mat')
        os.makedirs(os.path.dirname(mat_path), exist_ok=True)
        model.save_coeff(mat_path) # save predicted coefficients
